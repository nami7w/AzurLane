...
Gui, Add, CheckBox, x365 y%Tab1_Y% w80 h20 gAnchorsettings vShip_TargetElite checked%Ship_TargetElite% , 賽任艦隊
...
AutobattleList = 自動|半自動|關閉|
	Gui, Add, DropDownList, x265 y%Tab1_Y% w80 h100 vAutobattle gAnchorsettings, % MenuList(AutobattleList, Autobattle)
...
Gui, Add, DropDownList, x110 y%Tab1_Y% w150 h150 vBossAction gAnchorsettings, % MenuList(BossActionList, BossAction)
Tab1_Y += 35
...
Gui, Add, CheckBox, x50 y%Tab_Y% w125 h20 gAcademysettings vItem_Ex4_Box checked%Item_Ex4_Box%, 科技箱T4`(功勳)
...
Guicontrolget, Ship_TargetElite
...
Settimer, SaveAnchor3settings, -1
Critical, off
return
...
Settimer, SaveAcademysettings, -1
Critical, off
return
...
Settimer, SaveTechacademysettings, -1
Critical, off
return
...
if (Techacademy_Done) ;軍部研究室OK
{
	...
	Loop
	{
		...
		if (Find(x, y, 432, 588, 532, 648, TechPage_StartTech)) ;開始研發
		{
			...
			else if (k=1 and TechTarget_01) or (k=2 and TechTarget_02) or (k=3 and TechTarget_03) or (k=3 and TechTarget_03) or (k=4 and TechTarget_04) or (k=5 and TechTarget_05) or (k=6 and TechTarget_06) or (k=7 and TechTarget_07)
			{
				if (py!=Is_StartTech) 
				{
					message = %pj% 優先順序不符 (順序: %py%)。
...
{	
	...
	if (AnchorChapter=9 and AnchorChapter2=2)
		MapX1 := 500, MapY1 := 130, MapX2 :=1260, MapY2 := 670
	else
	...
	Loop, 100
	{
		...
		{
			TargetFailed := 1, TargetFailed2 := 1, TargetFailed3 := 1,	TargetFailed4 := 1, BossFailed := 0
		...
		if ((AnchorChapter="異色1" or AnchorChapter="異色2") and TargetFailed<1 and (Ship_Target1 or SearchLoopcount>9)) ;
		{
			if (GdipImageSearch(x, y, "img/SubChapter/target_4.png", 90, SearchDirection, MapX1, MapY1, MapX2, MapY2))
			{
				LogShow("嗶嗶～發現：航空艦隊！(異色格)！X : " x " Y: " y )
				xx := x 
				yy := y 
				Loop, 15
				{
		...
		if ((Plane_Target1 or SearchLoopcount>9) and Plane_TargetFailed1<1) ;航空器
		{
			if (GdipImageSearch(x, y, "img/SubChapter/target_plane1.png", 25, SearchDirection, MapX1, MapY1, MapX2, MapY2))
			{
				LogShow("嗶嗶～發現：航空器！X : " x " Y: " y )
				xx := x 
				yy := y 
				Loop, 15
				{
...
if (AcademyDone<1)
{
	...
	Loop
	{
		if (GdipImageSearch(x, y, "img/Academy/AcademyOil.png", 100, 8, 95, 298, 542, 723) and AcademyOil and GetOil<1) ;
		{
			LogShow("發現石油，高雄發大財！")
			GetOil := 1
			C_Click(x, y)
...
if (DormDone<1) ;後宅發現任務
{
	...
	Loop
	{
		...
		if ((GdipImageSearch(x, y, "img/Dorm/Dorm_Coin.png", 8, 8, DormX1, DormY1, DormX2, DormY2) or GdipImageSearch(x, y, "img/Dorm/Dorm_Coin2.png", 7, 8, DormX1, DormY1, DormX2, DormY2)) and DormCoin and Dorm_Coin<3) 
		{
			LogShow("收成傢俱幣")
			C_Click(x, y)
			Dorm_Coin++
		}
		else if ((GdipImageSearch(x, y, "img/Dorm/Dorm_heart.png", 5, 8, DormX1, DormY1, DormX2, DormY2) or GdipImageSearch(x, y, "img/Dorm/Dorm_heart2.png", 10, 8, DormX1, DormY1, DormX2, DormY2)) and Dormheart and Dorm_heart<3) 
		{
			LogShow("增加親密度")
			C_Click(x, y)
			Dorm_heart++
...
FindWay(x, y) {
	global
	if (AnchorChapter="墜落1" or AnchorChapter="墜落2") {
		if (Find(n, m, 439, 328, 539, 388, theWay_TooFar)) {
			LogShow("哎呀哎呀，前往敵方艦隊的路途過遠！")
			;~ sleep 3500
			IsDetect := 1
			DetectPos := 0
			IsFind := 0
			...
			{
				DetectPos+= 35
				s_x1 := if (x-DetectPos<=MapX1) ? MapX1 : (x-DetectPos)
				s_x2 := if (x+DetectPos>=MapX2) ? MapX2 : (x+DetectPos)
				s_y1 := if (y-DetectPos<=MapY1) ? MapY1 : (y-DetectPos)
				s_y2 := if (y+DetectPos>=MapY2) ? MapY2 : (y+DetectPos)
				Message := % "X1: " s_x1 " X2: " s_x2 " Y1: " s_y1 " Y2: " s_y2
				if GdipImageSearch(n, m, "img/SubChapter/MoveWay.png", 25, 8, s_x1, s_y1, s_x2, s_y2) 	{
					LogShow("前往: X " n " Y " m " 循環 " A_index " 次")
					m1 := m-10
					C_Click(n, m1)
					sleep 1000
					if (Find(q, w, 439, 328, 539, 388, theWay_TooFar)) {
						m1 := m+10
						C_Click(n, m1)
						sleep 1000
					}
			...
			if !(IsFind) {
				...
				C_Click(1025, 713)
				sleep 500
				if GdipImageSearch(n, m, "img/SubChapter/MoveWay.png", 25, 8, MapX1, MapY1, MapX2, MapY2)
				{
					LogShow("前往: X " n " Y " m " 循環 " A_index " 次")
					m1 := m-10
					C_Click(n, m1)
...
;~ Random, SearchDirection, 1, 8
;~ if (GdipImageSearch(x, y, "img/SubChapter/targetboss_1.png", 10, SearchDirection, MapX1, MapY1, MapX2, MapY2))
;~ {
;~ WinActivate, %title%
;~ mousemove, %x%, %y%
;~ tooltip x%x% y%y%
